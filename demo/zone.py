...
def pieces(img,coord):
    ...
    for c in contours:
      ...
      polygon_points = cv2.approxPolyDP(c, 0.02 * contour_length, True)
      area = cv2.contourArea(polygon_points)
      if area >= 16000 and area <= 17000:
         x, y, w, h = cv2.boundingRect(polygon_points)
         centerX = (x + w) / 2
         centerY = (y + h) / 2
         cv2.rectangle(color_img, (x, y), (x + w, y + h), (0, 255, 0), 2)
         cv2.putText(color_img, 'zone de piece detecte', (x+20 , y + 20), 0, 0.3, (0, 255, 0))
    ...
    for c in contours:
      ...
      if area > 20000 and area < 40000:
        ...
        # cv2.putText(color_img, 'zone de depart detecte', (centerX + 40, centerY + 50), 0, 0.3, (0, 255, 0))
         ...
         centerY = (y + h) / 2
         cv2.rectangle(color_img, (x, y), (x + w, y + h), (0, 255, 0), 2)
         cv2.putText(color_img, 'table detecte', (x+1220, y), 0, 0.3, (0, 255, 0))
